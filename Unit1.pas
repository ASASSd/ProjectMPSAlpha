...
uses
  ...
  Dialogs, ExtCtrls, StrUtils, StdCtrls,
...
private
  ...
  procedure Init();
  function strColor(const color:TColor):String;
...
begin
  ...
  With (GridCache) do
    ...
    for i:=0 to 30 do
      begin
      RowCount:=i;
      if(Cache[i].charged) then
        begin
        if(Cache[i].prediction) then
          begin
          Cells[0,i]:=(c16(Cache[i].Afrom,4));
          Cells[1,i]:='JUMP TO '+' '+c16(Cache[i].Ajump,4);
          end
        else
          begin
          Cells[0,i]:=(c16(Cache[i].Afrom,4));
          Cells[1,i]:='NO JUMP '+' '+c16(Cache[i].Ajump,4);
          end;
      end
    else
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if RResult.input < 0 then CF:=True;
      if RResult.input > 0 then PF:=True;
...
begin
  With cmdpipe[1] do
    ...
    begin
      Rcom.R1:=StrToInt('$'+LeftStr(c16(R_CONST,4),2));
      Rcom.R2:=StrToInt('$'+RightStr(c16(R_CONST,4),2));
    end
...
begin
  Init();
  ozuAdd(cmdMov(17,3),0);        // с нуля стартуем
  ozuAdd(cmdMov(34,7));
  ozuAdd(cmdAdd(3,7,8));
  ozuAdd(cmdSub(3,7,9));
  ozuAdd(cmdSub(9,3,10));
  ozuAdd(cmdJMP(7));
  ozuAdd(cmdMov(1,7));
  ozuAdd(cmdMov($FFFF,7));
  loadCache();
  ShowOzu();
end;
...
begin
  ...
  imgpath:=extractfiledir('D:\codezzz\RISC_SRC\images');
end;
...
function TFormM.strColor(const color:TColor):String;
begin
  if (color=clGreen)then Result:='green'
  else if color=clRed then Result:='red'
  else if color=clFuchsia then Result:='rose'
  else if color=clGray then Result:='grey'
  else if color=clBlue then Result:='blue'
  else Result:='not-exists-than-hide';
end;

procedure TFormM.st1(const xcolor:Tcolor);
var
  Color:String;          // Дописать для обращений в CACHE
begin
  color:=strColor(xcolor);
  panelpipe1.Color:=xcolor;
  setImg(q_pointer,color+'\pointer');  
  setImg(q_rk,color+'\rk');
  setImg(q_ozu,color+'\ozu');
  setImg(lines_pointer,'lines\'+color+'\pointer');
  setImg(lines_rk,'lines\'+color+'\rk');
  setImg(lines_cachedown,'lines\'+color+'\cachedown');
  if (cmdpipe[2].TypeOP=jump) and (jumpPrediction(cmdpipe[2].Adress)) then
    begin
    setImg(lines_cacheup,'lines\'+color+'\cacheup');
    PanelCacheOUT.Caption:=c16(Pcom,4);
    PanelCacheOUT.Color:=xcolor;
    setImg(q_cacheOUT,color+'\resout');
    end;
  PanelCacheIN.Color:=xcolor;
  setImg(q_cacheIN,color+'\resin');
...
begin
  ...
  setImg(q_dc,color+'\dc');
  setImg(lines_dc,'lines\'+color+'\dc-lines');
...
begin
  ...
  if not(op=jump) then setImg(lines_wb3,'lines\'+color+'\wb3');
  if(op=load) then
  begin
    setImg(lines_data_1,'lines\'+color+'\data1');
    PanelWWBS3.Color:=xcolor;
  end else if(op=exec) then
  begin
    setImg(q_bufin,color+'\bufin');
    PanelBUF1IN.Color:=xcolor;
    PanelBUF2IN.Color:=xcolor;
    if(back<>1) then
    begin
      setImg(lines_ron_a1,'lines\'+color+'\ron-a1');
      setImg(lines_ron_w1,'lines\'+color+'\ronw1');
      setImg(q_ron1,color+'\ron1');
    ...
    begin
      ...
      begin
        ...
        setImg(lines_ron_a2,'lines\'+color+'\ron-a2');
        setImg(lines_ron_w2,'lines\'+color+'\ronw2');
        setImg(q_ron2,color+'\ron2');
      end;
      if (cmdPipe[3].codeOP=7)or((cmdPipe[3].codeOP=8))then
      begin
        PanelRONAA.Color:=xcolor;
...
begin
  ...
  begin
    ...
    setImg(lines_jmp4,'lines\'+color+'\stage4jmp');
    PanelCacheIN.Color:=xcolor;
  ...
  if(op=exec) then
    begin
    setImg(q_bufout,color+'\bufout');
    setImg(q_resin,color+'\resin');
    PanelBUF1OUT.Color:=xcolor;
    PanelBUF2OUT.Color:=xcolor;
    setImg(lines_buf_alu,'lines\'+color+'\bufalu');
    PanelRESin.Color:=xcolor;
    setImg(q_alu,color+'\alu');
    if(back=1) then setImg(lines_alu,'lines\'+color+'\alu-left');
    if(back=2) then setImg(lines_alu,'lines\'+color+'\alu-right');
    if(back=0) then setImg(lines_alu,'lines\'+color+'\alu-noback');
    end
end;
...
begin
  ...
  if(op<>jump) then
    ...
    setImg(lines_wb5,'lines\'+color+'\wb5');
    setImg(q_ron3,color+'\ron3');
    PanelRONWWB.Color:=xcolor;
    PanelRONAWB.Color:=xcolor;
    end;
  if(op=load) then
  begin
    setImg(lines_data_2,'lines\'+color+'\data2');
    PanelWWBS5.Color:=xcolor;
  end else if(op=exec) then
    begin
      setImg(q_resout,color+'\resout');
      setImg(lines_w3,'lines\'+color+'\ronw3');
