...
type
  ...
  private
    ...
    procedure ShowOZU();
    procedure ECell(const r:Integer);
...
procedure TFormEditor.ECell(const r:Integer);
begin
//

GB.Caption:='ОЗУ[ '+GridOZU.Cells[0,r]+' ]';
ozuAdr:=StrToInt('$'+trim(GridOZU.Cells[0,r]));
...
hints.Add('JZ adr - Команда условного перехода по адресу adr в оперативной памяти. Переход выполняется, если результатом предшествовавшей арифметической операции на АЛУ был 0 (ноль).');
hints.Add('JC adr - Команда условного перехода по адресу adr в оперативной памяти. Переход выполняется, если результат предшествовавшей арифметической операции на АЛУ был отрицателен.');
...
hints.Add('JP adr - Команда условного перехода по адресу adr в оперативной памяти. Переход выполняется, если результат предшествовавшей арифметической операции на АЛУ был положителен.');
